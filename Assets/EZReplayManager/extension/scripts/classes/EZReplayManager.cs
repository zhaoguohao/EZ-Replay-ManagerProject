...
public sealed class EZReplayManager : MonoBehaviour {	
    ...
    protected Object2PropertiesMappingListWrapper DeSerializeObject(string filename)
    {
        ...
        //Debug.Log("System.GC.GetTotalMemory(): "+System.GC.GetTotalMemory(false));
        stream.Close();
    ...
    public void loadFromFile(string filename)
    {
        ...
        {
            if (showWarnings)
                Debug.Log("EZReplayManager WARNING: The EZR version with which the file has been created differs from your version of the EZReplayManager. This can cause unintended behaviour.");
        }
        ...
        if (currentMode == ViewMode.LIVE)
        {
            if (currentAction != ActionMode.PLAY)
            {
                ...
                { //if not already existant	
                    ...
                    if (showHints)
                    Debug.Log("EZReplayManager HINT: GameObject '" + go + "' has already been marked for recording.");

            }
            else
                if (showWarnings)
                Debug.Log("EZReplayManager WARNING: You cannot mark GameObject '" + go + "' for recording while a recording is being played.");
        }
        else
        {
            if (showWarnings)
                Debug.Log("EZReplayManager WARNING: You cannot mark GameObject '" + go + "' for recording while in replay mode.");
    ...
    public void switchModeTo(ViewMode newMode)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (showWarnings)
                    Debug.Log("EZReplayManager WARNING: You have not recorded anything yet. Will not replay.");
            }
    ...
    public void record()
    {
        ...
        if (currentMode == ViewMode.LIVE)
        {
            ...
            {
                if (showWarnings)
                    Debug.Log("EZReplayManager WARNING: Ordered to record when recorder was not in stopped-state. Will not start recording.");
            }
        }
        else
        {
            if (showWarnings)
                Debug.Log("EZReplayManager WARNING: Ordered to record when recorder was in replay mode. Will not start recording.");
        }
    ...
    public void pause()
    {
        if (currentMode == ViewMode.REPLAY)
        {
            ...
            if (showWarnings)
            Debug.Log("EZReplayManager WARNING: Ordered to pause when recorder was not in replay mode. Will not pause.");
    ...
    public void play(int speed, bool playImmediately, bool backwards, bool exitOnFinished)
    {
        ...
        if (currentAction == ActionMode.STOPPED || currentAction == ActionMode.PAUSED)
        {
            ...
            if (showHints)
            Debug.Log("EZReplayManager HINT: Ordered to play when not in stopped or paused state.");
    }
